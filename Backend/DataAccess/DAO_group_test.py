...
class DAO_group_test:
    ...
    def get_group_test_calendar(self, role: str, user_id: str, start: datetime, end: datetime) -> list[GroupTest]:
        with get_MS_database(True) as cursor:
            if role == 'teacher':
                sql = """
                    SELECT [start], [end], [duration], [name]
                    FROM [group_test] 
                    WHERE [group_id] IN (SELECT [id] FROM [group] WHERE [teacher_id] = %s) AND CAST([start] AS DATE) >= CAST(%s AS DATE) AND CAST([start] AS DATE) <= CAST(%s AS DATE)
                """
            elif role == 'student':
                sql = """
                    SELECT [start], [end], [duration], [name]
                    FROM [group_test]
                    WHERE [group_id] IN (SELECT [group_id] FROM [group_student] WHERE [student_id] = %s) AND CAST([start] AS DATE) >= CAST(%s AS DATE) AND CAST([start] AS DATE) <= CAST(%s AS DATE)
                """
    ...
    def insert_group_test(self, data: GroupTest) -> str:
        ...
        with get_MS_database(False) as cursor:
            while True:
                try:
                    cursor.execute(
                        "INSERT INTO [group_test]([id], [group_id], [test_id], [name], [start], [end], [duration], [shuffle], [hash_pswd], [tolerance], [n_page], [allow_move])  VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (id, data.group_id, data.test_id, data.name, data.start, data.end, data.duration, data.shuffle, data.hash_pswd, data.tolerance, data.n_page, data.allow_move))
                    return id
