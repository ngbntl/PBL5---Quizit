...
from Backend.Model.response_model import Res_Group
...
async def get_groups(teacher: Annotated[Teacher, Depends(get_current_user)],
    ...
    try:
        return [Res_Group(**row) for row in group_service.get_groups_by_teacher(teacher.id, is_show)]
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))


@teacher_group_router.get('/{group_id}', response_model=Res_Group, status_code=status.HTTP_200_OK)
async def get_group_by_id(teacher: Annotated[Teacher, Depends(get_current_user)],
                          ...
                          group_service: Annotated[group_business, Depends()]):
    try:
        return Res_Group(**group_service.get_group_by_id(group_id, teacher_id=teacher.id))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def get_students_in_group(_: Annotated[Teacher, Depends(get_current_user)],
    ...
    try:
        return group_student_service.get_students_by_group(group_id, join)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))


@teacher_group_router.patch('/student', status_code=status.HTTP_204_NO_CONTENT)
async def accept_request(teacher: Annotated[Teacher, Depends(get_current_user)],
                         group_id: Annotated[str, Query(min_length=8, max_length=8)],
                         student_id: Annotated[str, Query(min_length=8, max_length=8)],
                         group_student_service: Annotated[group_student_business, Depends()]):
    try:
        group_student_service.accept_join_request(teacher.id, group_id, student_id)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
@student_group_router.post('/{group_id}', status_code=status.HTTP_201_CREATED)
...
@student_group_router.patch('/{group_id}', status_code=status.HTTP_204_NO_CONTENT)
